...
**SPP Score: 7.1 / 10** - 75W max with built-in retractable USB-C cable, ultra-compact. Our all-round pick.
...
**SPP Score: 6.9 / 10** - 66W total (USB-C 36W plus USB-A 30W), PD3.0, 12V cigarette lighter, metal adapter. Our value pick.
...
**SPP Score: 8.0 / 10** - 150W total PD140W (USB-C), 4 ports, cigarette lighter adapter. Our pick for maximum capability.
...
We started with what actually limits a car charger: the 12V cigarette lighter socket and the fuse behind it. Most UK cars fuse that socket at 10A or 15A, which caps real-world draw at around 120 to 180 watts regardless of what the charger is rated for. With that ceiling in mind, we picked chargers that cover three different load shapes: a single phone (the Anker), a phone plus a second device (the INIU), and a laptop plus everything else (the UGREEN). We checked each ASIN against its live Amazon UK listing the day we wrote this, scored every pick on our published rubric, and left port counts and wattage claims as the makers list them. We take no payment for placement and we do not display any data we have not verified ourselves. If you need a [cable for the USB-C port](best-usb-c-cable-for-fast-charging.html), our cable guide covers that end.
